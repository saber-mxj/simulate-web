    ...
    connecting: {
      ...
      validateMagnet({ magnet, cell }) {
        const inputPortInfo = cell
          .getData()
          .ports.filter(
            (portItem) => portItem.type === NsGraph.AnchorType.INPUT,
          );
    ...
    IModelContribution
{
  ...
  registerHook = async (hooks: ICmdHooks) => {
    ...
    const disposables = [
      ...
      hooks.addEdge.registerHook({
        name: 'dag-add-edge',
        handler: async (args) => {
          const { layout = LayoutEnum.TOP_BOTTOM } =
            await this.propConfig.getConfig();
  ...
  registerHookHub = async (registry: IHookService) => {
    return registry.registerHookHub(
      NsGraphStatusCommand.hookKey,
      new HookHub(),
    );
  };
